...
class MaxAndSkipEnv(Wrapper):
    def __init__(self, env=None, skip=4):
        ...
        self._skip = skip
        #self._initial_lives = None  # add lives

    def step(self, action):
        #if self._initial_lives is None:
        #   self._initial_lives = 10 # get initial lives
...
class CustomReward(Wrapper):
    ...
    def step(self, action):
        ...
        # for every enemie, reward +40
        if 'score' in info:
            reward += (info['score'] - self._current_score) * 40.0
        
        # for every coin, reward +30
        if 'coins' in info:
            reward += info['coins'] * 55.0
        ...
        if done:
            ...
            #if info['life'] < self._initial_lives:  
                #reward -= 400.0 
