...
def make_action(screen, info, step, env, prev_action):
    ...
    # Determine Mario's location

    # get mario location in screen
    if mario_locations:
        mario_x, mario_y = mario_locations[0][0]
        mario_w, mario_h = mario_locations[0][1]  # width and height
    else:
        return prev_action

    # position of mario world
    mario_world_x = info["x_pos"]
    mario_world_y = info["y_pos"]
    ...
    for enemy in enemy_locations:
        ...
        enemy_w, enemy_h = enemy[1]

        if 0 <= enemy_x - (mario_x + mario_w) <= 35 and 0 <= (enemy_y + enemy_h) - (mario_y + mario_h) <= 10:
            print("jump enemy!")
            return 2
    ...
    for block in block_locations:
        ...
        block_w, block_h = block[1]
        block_name = block[2]
        # pipe
        if block_name == "pipe" and 0 <= block_x - mario_x < 45:
            print("jump pipe!")
            return 2
        # question block
        if block_name == "question_block" and 0 <= block_x - (mario_x + mario_w) < 20 and mario_y - (
                block_y + block_h) < 80 and (1600 > mario_world_x or mario_world_x > 1800):
            print("hit question_block!")
            return 2
        # identify block obstacles in front of Mario
        if (block_name == "block"
                and -5 <= block_y + block_h - mario_y - mario_h <= 5
                and -5 <= block_x - mario_x - mario_w <= 5):
            global jump_counter
            jump_counter = 3
    # block obstacles need hold jump button
    if jump_counter > 0:
        jump_counter -= 1
        print(jump_counter)
        print("jump block")
    ...
    # handle cliff
    is_cliff = False
    nearest_block_x = None

    for block in block_locations:
        block_x, block_y = block[0]
        block_w, block_h = block[1]

        if block_x > mario_x + mario_w:
            if nearest_block_x is None or block_x < nearest_block_x:
                if abs(block_y - mario_y) > 10:
                    nearest_block_x = block_x + block_w

    # identify cliff
    if nearest_block_x is not None:
        gap_width = nearest_block_x - (mario_x + mario_w)

        if 35 <= gap_width < 100:
            is_cliff = True

    if is_cliff:
        print(" jump over cliff!")
        return 2
